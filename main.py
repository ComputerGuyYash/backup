...
for file in req:
	...
	print(str(video_number)+"/"+len_req)
	...
	while(cap.isOpened()):
		...
		if(current_frame==fn):
			...
			try:
				...
				this = [audio_weight,region_weight,ffcolor_weight,toggle(int(float(str(CNN_weight[0][0]))))]
				...
				#print(this)
				final = np.array([this])
				Final_weight = MAIN_model.predict(final)
				print(Final_weight)
				exit = [file,str(Final_weight[0][0])]
				submission.append(exit)
				#submission = np.vstack((submission,exit))
				#print(submission)
